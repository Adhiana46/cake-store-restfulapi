...
type GetCakeListRequest struct {
	Limit     *int     `query:"limit" form:"limit" validate:"omitempty,numeric,min=1,max=100"`
	Page      *int     `query:"page" form:"page" validate:"omitempty,numeric,min=1"`
	S         *string  `query:"s" form:"s" validate:"omitempty,min=3,max=100"`
	RatingMin *float32 `query:"rating_min" form:"rating_min" validate:"omitempty,numeric,gte=0,lte=10"`
	RatingMax *float32 `query:"rating_max" form:"rating_max" validate:"omitempty,numeric,gte=0,lte=10"`
	SortBy    *string  `query:"sort_by" form:"sort_by" validate:"omitempty"`
}
